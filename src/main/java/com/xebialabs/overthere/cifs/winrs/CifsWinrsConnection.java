...
public class CifsWinrsConnection extends CifsConnection {
    ...
    public OverthereProcess startProcess(final CmdLine commandLine) {
        ...
        try {
        	...
        	winrs.addArgument("-remote:" + createWinrmURL(options));
        	winrs.addArgument("-username:" + username);
        	winrs.addPassword("-password:" + password);
            if (workingDirectory != null) {
            	winrs.addPassword("-directory:" + workingDirectory);
            }
        	...
        	logger.debug("Invoking local command [{}]", winrs);
            final ProcessBuilder pb = new ProcessBuilder(winrs.toCommandArray(WINDOWS, false));
